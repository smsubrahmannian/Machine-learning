...
def perform_regularised_cv(train,y_colname, grid,high_card_cols, folds = 5,metric = mean_absolute_error):
    ...
    for params in tqdm(param_grid):
        ...
        for train_idx, test_idx in kf.split(train):
            ...
            _,error,_,_ = train_model(params,kf_train,kf_test,y_colname,high_card_cols,valid=True,metric= metric)
            errors.append(error)
...
def train_model(params,train,test,y_colname,cols_to_mean_enc,valid=None,metric=mean_absolute_error):
    ...
    rf.fit(train.drop([y_colname], axis=1), train.loc[:,y_colname])
    metric_eval = None
    if valid:
        preds = rf.predict(test.drop([y_colname], axis=1))
        metric_eval = metric(test[y_colname], preds)
    return rf, metric_eval,train,test
...
def n_estim_calculate(model,df):
    ...
    preds = np.stack([t.predict(df.drop('salary',axis=1)) for t in model.estimators_])
    plt.plot([mean_squared_error(y_pred=np.mean(preds[:i+1],axis=0), y_true=df['salary']) for i in range(0, len(model.estimators_))])
...
def feature_importance(df,m):
    ...
    '''    
    fi = pd.DataFrame({'cols': df.drop('salary',axis=1).columns, 'importance': m.feature_importances_})
...
def partial_dependence(X, fldname,m,categorymap):
    ...
    final['level_0']=final['level_0'].map(dict(categorymap))
